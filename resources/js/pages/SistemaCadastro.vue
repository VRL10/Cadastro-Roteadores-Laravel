...
<!-- Variáveis referente a Repartição -->
type Reparticao = {
    id: number;
    nome_contato: string;
...
<!-- Variáveis referente a Roteador -->
type Roteador = {
    id: number;
    ip_roteador: string;
...
<!-- Variáveis referente a MacAddress -->
type Mac = {
    id: number;
    mac_address: string;
    nome_usuario: string;
    ...
    <script setup lang="ts">
    import { Head } from '@inertiajs/vue3';
    import SistemaCadastroConteudo from '@/components/SistemaCadastroConteudo.vue';
    import { useSistemaCadastro } from '@/composables/useSistemaCadastro';

    const sistema = useSistemaCadastro();
    </script>

    <template>
        <Head title="Sistema de Cadastro" />

        <SistemaCadastroConteudo :sistema="sistema" />
    </template>
                </div>
